...
# # Put your Piper voice files here (you must download them)
# PIPER_VOICE_ONNX = MODELS / "piper_voice" / "voice.onnx"
# PIPER_VOICE_JSON = MODELS / "piper_voice" / "voice.onnx.json"

# # LLM: easiest MVP is to have llama-cli pull from HF each run (it caches)
# LLAMA_HF_REPO = "bartowski/Llama-3.2-3B-Instruct-GGUF"
# LLAMA_HF_FILE = "Llama-3.2-3B-Instruct-Q4_K_M.gguf"
...
#     """
#     Runs whisper-cpp CLI on a wav and returns the transcript.
#     """
#     require_file(WHISPER_MODEL, "Download ggml-base.en.bin into ./models (see instructions).")

#     out_base = wav_path.with_suffix("")  # whisper-cpp uses -of as base path
#     txt_path = Path(str(out_base) + ".txt")

#     # whisper-cpp CLI commonly supports:
#     # -m model.bin
#     # -f input.wav
#     # -otxt (output .txt)
#     # -of output_base
...
#     # whisper outputs may include leading timestamps depending on flags; keep it simple for MVP:
#     return text
...
#     require_file(LLM_MODEL, "Download the GGUF into ./models/llm/ (see earlier curl command).")
...
#         "--temp", "0.7",
#         "--top-p", "0.9",
#         # stop before it starts inventing a new user turn:
#         "--stop", "\nUser:",
#         "--stop", "\nSystem:",
#         "--stop", "\nAssistant:",
#         prompt,
...
#     # If anything echoed the prompt, keep only after the last Assistant:
#     if "Assistant:" in text:
#         text = text.split("Assistant:")[-1].strip()

#     # Remove special tokens if they appear
#     text = text.replace("<|begin_of_text|>", "").strip()

#     return text or "Hmm—can you say that again?"
...
#     # Play wav using macOS built-in player (reliable)
...
#             print("Speaking...")
#             print("DEBUG: about to speak, reply length =", len(reply))
#             tts_piper(reply)
